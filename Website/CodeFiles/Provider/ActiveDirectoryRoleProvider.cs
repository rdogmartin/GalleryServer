...
namespace GalleryServer.Web
{
    ...
    public sealed class ActiveDirectoryRoleProvider : RoleProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            // Extract everything after the "DC=" and assign to variable (e.g. "DC=mydomain,DC=techinfosystems,DC=com")
            ...
            // Now find the domain server. It's between the LDAP:// and the next /
            // LDAP://192.168.1.1/CN=users,DC=mydomain,DC=techinfosystems,DC=com
            // In this example, it's 192.168.1.1
            // NOTE: In 4.4.2 and earlier, we assumed the domain was the value of the first "DC=" portion, but that didn't work for a customer.
            const string ldap = "LDAP://";

            idx = _connectionStringName.IndexOf("/", ldap.Length, StringComparison.Ordinal);
            if (idx >= 0)
            {
                _domain = _connectionStringName.Substring(ldap.Length, idx - ldap.Length);
            }
            else
            {
                throw new ProviderException("The connection string specified in 'connectionStringName' does not appear to be a valid LDAP connection string. Expected to find '/' separating the domain name from the rest of the connection string.");
            }
